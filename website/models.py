...
class Profile(models.Model):
    ...
    referred_by = models.ForeignKey(
        "Profile", null=True, related_name="referred", on_delete=models.SET_NULL
    )
    referral_code = models.CharField(
        max_length=50, unique=True, default=generate_referral_code
    )
    ...
    )  # can be school,college. last institute for professionals
    ...
    def get_or_set_profile_status(self, toSet=False):
        ...
        if profile_status and email_status:
            if self.referred_by is not None:
                referral, _ = ValidReferral.objects.get_or_create(
                    by=self.referred_by, to=self
                )
...
class ValidReferral(models.Model):
    by = models.ForeignKey(
        Profile, on_delete=models.CASCADE, related_name="referred_people"
    )
    to = models.OneToOneField(
        Profile, on_delete=models.CASCADE, related_name="referral"
    )
...
class Team(models.Model):
    ...
    event = models.ForeignKey(Event, on_delete=models.CASCADE)
    registration_time = models.DateTimeField(auto_now_add=True, default=datetime.datetime.now())
    members = models.ManyToManyField(
        Profile, through="Membership", related_name="team_members"
    )
    creator = models.ForeignKey(
        Profile, related_name="teams_created", on_delete=models.CASCADE
    )
...
class Handles(models.Model):
    profile = models.OneToOneField(
        Profile, on_delete=models.CASCADE, related_name="handles"
    )
